...
echo -e "\nSetup Jobs ..."
echo "... weekly fstrim"
sudo ln -s ~/config/scripts/jobs/fstrim.sh /etc/cron.weekly/fstrim
echo "... weekly os-rsync"
backupDir=/mnt/backup
[ ! -d "$backupDir" ] && read -p "Backup dir: " backupDirTo \
  && [ -d "$backupDirTo" ] && sudo ln -s $backupDirTo $backupDir
if [ -d "$backupDir" ]; then
  sudo ln -s ~/config/scripts/jobs/os-rsync.sh /etc/cron.weekly/os-rsync
else
  echo "skipped, no /mnt/backup linked"
fi
echo "... hourly zfs-health"
if $hasZFS; then
  sudo ln -s ~/config/scripts/jobs/zfs-health.sh /etc/cron.hourly/zfs-health
else
